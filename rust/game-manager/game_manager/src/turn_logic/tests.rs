...
mod test_order {
    ...
    fn sort_intents_correctly() {
        ...
        // let action = Ac
        let i1 = Intent::test_intent(None, Some(0), None, Some(0));
        let i2 = Intent::test_intent(None, Some(1), None, Some(1));
        let i3 = Intent::test_intent(None, Some(2), None, Some(2));
        im.submit(i2);
        im.submit(i3);
        im.submit(i1);
        // dbg!(im);
        assert_eq!(im.queue[0].priority, 0);
        assert_eq!(im.queue[1].priority, 1);
        assert_eq!(im.queue.last().unwrap().priority, 2);
    }
    #[test]
    fn new_intents_gets_treated_last() {
        let mut im = IntentManager::new();
        let old = Intent::test_intent(None, Some(0), None, Some(0));
        let new = Intent::test_intent(None, Some(0), None, Some(1));
        im.submit(old);
        im.submit(new);
        assert_eq!(im.queue.last().unwrap().entity.unique_id, EntityId(0));
    }
    #[test]
    fn remainder_intents_gets_treated_first() {
        let mut im = IntentManager::new();
        let action = Action::test_action(ActionKind::Move);
        let both_intents_processed_first =
            Intent::test_intent(Some(action.clone()), Some(0), None, Some(23));
        let other = Intent::test_intent(Some(action), Some(0), None, Some(55));
...
mod test_extract_top_intent {
    ...
    fn priority_kept_for_remainder() {
        let intent: Intent = Intent::test_intent(None, Some(5), None, None);
